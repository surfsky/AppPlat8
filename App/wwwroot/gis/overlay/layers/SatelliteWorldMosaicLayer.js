...
export class SatelliteWorldMosaicLayer extends MapLayer {
  ...
  async refresh() {
    try {
      ...
      console.error("刷新全球卫星云图失败", e);
      const msg = e instanceof Error ? e.message : String(e || "全球卫星云图加载失败");
      this.setLastError(msg || "全球卫星云图加载失败");
      this.clearDataTime();
      this.setInfoExtra("");
  ...
  setOpacity(opacity) {
    ...
    if (map.getLayer(this.layerId)) {
      map.setPaintProperty(this.layerId, "raster-opacity", safe);
    }
    this.opacity = safe;
  }
