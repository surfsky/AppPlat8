...
1. 数据库迁移: 运行 `dotnet ef migrations add FeedbackReplyImage --project App/App.csproj --startup-project App/App.csproj`.
...
- [X] 平台框架
  - [X] 图标
  - [X] API 框架：HttpApi 集成了API服务、缓存、接口文档库、接口测试、接口监管和流量控制等功能；
  - [X] EleUI控件
    - [X] Form
    - [X] Table
    - [X] Control
...
- [ ] 驾驶舱
  ...
  - [X] 点位
    ...
    - [ ] 支持多种展示方式：
      ...
      - [X] 三维建筑模型：
        ...
        - [X] 在地图上直接展示三维模型；
      - [X] 监控：在地图上显示点，点击后可播放监控视频
      - [X] 区域图片：在地图上直接展示，会随着Zoom级别变化而自动拉伸显示
      - [X] 各种常见文档的弹窗展示：Word、Excel、PDF、Markdown、脑图、视频、全景图、三维模型等
      - [ ] 金字塔图片：多分辨率金字塔图片图层；
  - [X] 气象图层
    - [X] 降雨雷达图
    - [X] 卫星云图
  ...
  - [X] 统计面板
    - [ ] 区域：
      - [ ] 屏幕分成左、右、中上、中、中下几个区域，区域内流式布局，可定义Size、流式布局方向FlowDirection、间距Gap、边距Margin、滚动方向ScrollDirection等；
      - [ ] 接口返回这些区域的json配置信息；
      - [ ] 驾驶舱页面对这些区域进行响应式布局；
    - [X] 面板
      - [X] 基础信息：区域、尺寸、提示等
...
- [ ] 后台任务(App.Scheduler)
  ...
  - [X] 定时任务管理：根据需求设置定时任务，如单据定时自动流转、统计数据、发送邮件等
  - [X] 任务执行：根据任务配置，定时执行任务，可查看进度、日志等
  - [ ] 细化各种任务，如数据清洗、统计、发送告警通知等；
- [ ] AI 能力
  - [X] AI 简单对答
  - [ ] AI 上传图片分析
  - [ ] AI 访问知识库，进行文档检索问答
  - [ ] AI 使用 MCP 访问业务数据，进行问答（需要解决授权问题）
  - [ ] AI 驾驶舱，根据对答自动展示相关资源（需将系统功能模块的接口暴露给AI调用，并展示在驾驶舱上）
  - [ ] 提供MCP服务供其它网站调用
- [ ] 部署
  - [X] 本地部署
  - [X] 内网穿透测试
...
安全生产与消防安全领域数字化平台已完成：
（1）基础网站框架（部门、账户、角色、权限、菜单、登录、后台、欢迎页等）；
（2）运维框架（日志审计、HttpAPI框架、站点配置等）；
（3）U I框架（基于ElementPlus的响应式 RazorPage 控件库）；
（4）办公模块（资产、预算、公告、企业、交办任务、事件、项目管理等）；
（5）知识库（文档、目录、各文档在线展示等）；
（6）隐患排查模块（检查对象、检查表、风险点、隐患排查流程、检查任务等）；
（7）应急指挥驾驶舱框架模块（地图、视图、场景、地址检索、统计面板、图层面板、数据面板、图层菜单管理、点位管理和标绘、各种点位附件文档展示、气象图层等模块）；
（8）AI集成（AI对话及隐患识别）

待完成或待优化任务：
（1）欢迎页面（消息公告、任务及进度）
（2）隐患排查（隐患复查和核查、隐患任务、任务统计及后台定时调度引擎、各类统计报表）
（3）驾驶舱优化（金字塔地图图层、气温斑点图层、降雨斑点图层、降雨预测图层、资规局最新地图、气象局数据、物联设备、各种统计面板和图表）；
（4）工作流（引擎、管理、业务流程和实例管理）
（5）AI能力（AI知识库、使用MCP协议访问数据库、以问答方式进行数据检索和整理展示及交互、内嵌AI Agent操作驾驶舱页面、提供MCP服务供其它网站调用）;
（6）部署和云主机；
（7）安可适配（数据库、中间件等）；
（8）安全防护（防火墙、数据库）；
（9）性能优化（高并发数据库、数据库分表分区、接口缓存、负载均衡、宽表设计和定时调度填充、报表查询后台调度方案等）；
