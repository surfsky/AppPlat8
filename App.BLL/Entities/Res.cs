...
namespace App.Entities
{
    ...
    /// </summary>
    [UI("系统", "附件")]
    public class Att : EntityBase<Att>, ISort
    {
        ...
        // 覆盖几个属性，避免递归循环
        [NotMapped, JsonIgnore]            public new List<Att> Atts { get; set; } = null;
        ...
        // 构造函数
        public Att(){}
        public Att(AttType type, string key, string content, string fileName, bool? protect)
        {
            this.Type = type;
            this.Key = key;
        ...
        public override void BeforeSave(EntityOp op)
        {
            ...
            HandleFile(this.Content);
        }

        /// <summary>处理文件资源</summary>
        public void HandleFile(string url)
        {
            // 文件扩展名及推断属性
            this.FileExtension = url.GetFileExtension();
            this.MimeType = IO.GetMimeType(this.FileExtension);
            this.Type = this.MimeType.Contains("image") ? AttType.Image : AttType.File;
        ...
        // 新增
        public static void Add(AttType type, string key, string content, string fileName = "", bool protect=true)
        {
            Att res = new Att(type, key, content, fileName, protect);
            res.Save();
        }
        ...
        public static void DeleteBatch(string prefix, List<long> ids)
        {
            foreach (int id in ids)
                Att.DeleteBatch(BuildUniId(prefix, id));
        }

        // 检索
        public static IQueryable<Att> Search(string key, AttType? type = null)
        {
            IQueryable<Att> q = Set;
